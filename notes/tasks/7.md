...
1. **Create `TreeState` class** (`src/treeState.js`): Deals with all states that affect tree appearance at a given size, independent of controls, legends, and actual tree display
  - Extends `Subscribable`
  - Constructor accepts initial options
  - Properties:
    - layout: 'circular', or 'rectangular'
    - label text source metadata column ID
    - label color source metadata column ID
    - label size source metadata column ID
    ...
    - occupied view width and height in pixels (might need to be different that the target)
    - label size scale (`ContinuousSizeScale`)
    - label color scale (`ContinuousColorScale` or `CategoricalColorScale`)
    - A copy of the tree hierarchy with the following appended to the `data` of each node: 
      - `isCollapsedSubtree`: which nodes are collapsed subtrees
      - `isCollapsedRoot`: which nodes are collapsed roots
      - `nodeLabel`: the node label
      - `tipLabel`: the tip label
      - `tipLabelFont`: label font
      - `tipLabelStyle`: label font style (bold, italic, normal)
      - `tipLabelSize`: label font size in pixels
      - `tipLabelColor`: label color as a hex code
      - `lengthPx`: the branch length in pixels
      - `angle`: the branch angle in radians
      - `radiusPx`: the node radius in pixels
      - `cos`: cached value of `Math.cos(angle)` in pixels
      - `sin`: cached value of `Math.sin(angle)` in pixels
      - `x`: the node x coordinate in pixels, where the root is at 0
      - `y`: the node y coordinate in pixels, where the root is at 0
      - `bounds`: the coordinates of the shape bounding the subtree and its labels in pixels or radians
        - `minX`
        - `maxX`
        - `minY`
        - `maxY`
        ...
        - `maxRadius`
  - Methods:
    - `setLayout(layoutType)` - toggle circular/rectangular. calls `updateScaling`
    - `setTipLabelTextSource(column)` - change label text. calls `updateScaling`
    - `setTipLabelColorSource(column)` - change label coloring. notify subscribers of font change
    - `setTipLabelSizeSource(column)` - change label size. calls `updateScaling`
    - `setTipLabelFontSource(column)` - change label font. calls `updateScaling`
    - `setTipLabelStyleSource(column)` - change label font style. notify subscribers of font change
    - `setTargetTreeDimensions(width, hight)` - change the ideal dimensions for the tree. calls `updateScaling`
    - `setLabelOffset` - change the label offset. calls `updateScaling`
    - `getTreeDimensions()` - get the height and width of the tree when plotted. derived from the shape bounding the root node
    - `updateScaling()` - update scaling factors. calls `updateCoordinates`
    - `updateCoordinates()` - update values appended to tree nodes that are in pixels or radians. notify subscribers of tree change
    - `collapseSubtree(node)` - collapse a subtree. calls `updateScaling`
    - `expandSubtree(node)` - expand a subtree. calls `updateScaling`
    - `collapseRoot(node)` - restrict the tree to the node. calls `updateScaling`
    - `expandRoot()` - expand the last collapsed root. calls `updateScaling`
